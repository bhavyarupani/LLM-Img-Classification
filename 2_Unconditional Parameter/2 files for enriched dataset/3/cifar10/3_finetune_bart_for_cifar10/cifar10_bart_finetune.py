...
def compute_semantics_2(extra_data: dict, strC: str, is_training: bool) -> int:
    ...
    # Random noise for better training
    ...
    with open(path, 'r') as f:
        for line in tqdm(f, desc=f"Processing {mode} data from {path}", dynamic_ncols=True):
            ...
            extra_string = (
                ...
                f"global_ref_score={int(extra_data['global_ref_score'])}, "
                f"similarity={extra_data['similarity_score']:.4f}"
            )
            prompt = template.format(a=strA, b=strB, extra=extra_string, m=semantic)

            # Append the record using the semantic metric from fields[3] for labels and semantic_metric.
            records.append({
                "input_text": prompt,
                "labels": fields[3],
...
train_df, train_metrics = load_data(train_data_path, mode="train")
test_df, test_metrics = load_data(test_data_path, mode="test")

all_metrics = sorted(train_metrics.union(test_metrics))
metric_to_id = {m: i for i, m in enumerate(all_metrics)}

train_df["labels"] = train_df["labels"].map(metric_to_id)
test_df["labels"] = test_df["labels"].map(metric_to_id)
...
model_name = f"cifar10_bart_finetuning_{timestamp}"
